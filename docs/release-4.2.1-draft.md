...
- Custom servers reject complete URLs and show an error beside the form. Enter a hostname or IP address, with its port in the separate Port field.
...
- The transition notice has one primary action, while retaining both purchase and existing-key options.
